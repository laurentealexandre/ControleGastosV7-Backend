...
const transportesController = {
    create: async (req, res) => {
        try {
            ...
            const [result] = await pool.query(
                `INSERT INTO lancamentos_transportes 
                (data, justificativa, solicitante, pcdp, valor, usuario_criacao) 
                VALUES (?, ?, ?, ?, ?, ?)`,
                [data, justificativa, solicitante, pcdp, valor, req.userId]
            );
    ...
    getAll: async (req, res) => {
        try {
            ...
            let query = `
                ...
                FROM lancamentos_transportes l
    ...
    update: async (req, res) => {
        try {
            ...
            const [result] = await pool.query(
                `UPDATE lancamentos_transportes 
                SET data = ?, justificativa = ?, solicitante = ?, pcdp = ?, valor = ? 
                WHERE id = ?`,
                [data, justificativa, solicitante, pcdp, valor, id]
            );
    ...
    delete: async (req, res) => {
        try {
            const { id } = req.params;
            const [result] = await pool.query(
                'DELETE FROM lancamentos_transportes WHERE id = ?',
                [id]
            );
...
module.exports = transportesController;