...
const manutencaoPredialController = {
    create: async (req, res) => {
        try {
            ...
            const [result] = await pool.query(
                `INSERT INTO lancamentos_manutencao_predial 
                (data, servico, numero_chamado, valor, usuario_criacao) 
                VALUES (?, ?, ?, ?, ?)`,
                [data, servico, numero_chamado, valor, req.userId]
            );
    ...
    getAll: async (req, res) => {
        try {
            ...
            let query = `
                ...
                FROM lancamentos_manutencao_predial l
    ...
    update: async (req, res) => {
        try {
            ...
            const [result] = await pool.query(
                `UPDATE lancamentos_manutencao_predial 
                SET data = ?, servico = ?, numero_chamado = ?, valor = ? 
                WHERE id = ?`,
                [data, servico, numero_chamado, valor, id]
            );
    ...
    delete: async (req, res) => {
        try {
            const { id } = req.params;
            const [result] = await pool.query(
                'DELETE FROM lancamentos_manutencao_predial WHERE id = ?',
                [id]
            );
...
module.exports = manutencaoPredialController;