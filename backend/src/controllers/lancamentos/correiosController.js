...
const correiosController = {
    create: async (req, res) => {
        try {
            ...
            const [result] = await pool.query(
                `INSERT INTO lancamentos_correios 
                (data, valor, solicitante, justificativa, destino, usuario_criacao) 
                VALUES (?, ?, ?, ?, ?, ?)`,
                [data, valor, solicitante, justificativa, destino, req.userId]
            );
    ...
    update: async (req, res) => {
        try {
            ...
            const [result] = await pool.query(
                `UPDATE lancamentos_correios 
                SET data = ?, valor = ?, solicitante = ?, 
                    justificativa = ?, destino = ? 
                WHERE id = ?`,
                [data, valor, solicitante, justificativa, destino, id]
            );
    ...
    delete: async (req, res) => {
        try {
            const { id } = req.params;
            const [result] = await pool.query(
                'DELETE FROM lancamentos_correios WHERE id = ?',
                [id]
            );
